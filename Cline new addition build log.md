...
| 14 | 2026-09-12 | 🚧 OPEN ISSUE — boss reports splash "not working" (parked for tomorrow) | ~2k | **HANDOFF:** Boss clicked classic→plus, no interstitial appeared. Server-side was verified OK before handoff (HTML carries splash + 10 letters + 3070ms duration; classic toggle link carries ?welcome=1; no ClientRouter/view-transitions in either layout, so the inline script should fire on nav; splash correctly self-removes without the param). ONE REAL BUG FOUND AT HANDOFF: JS adds `.ss-lock` to <html> but the CSS rule was never written → scroll never locks (fix = add `.ss-lock { overflow: hidden; }` to SkinSplash styles — 1-liner, but NOT the show-stopper). TOMORROW'S PLAN: (1) add ss-lock rule; (2) repro in incognito/hard-refresh (stale cached classic page = link without ?welcome=1 = prime suspect); (3) devtools: does #skinSplash get data-active='true'? any console errors? does /plus?welcome=1 typed manually play it? (4) test BOTH dev server (astro dev --background) and prod dist — boss may have been on dev while I verified prod. Do not rebuild variants until root cause confirmed. |

## 8b. HANDOFF — first move next session
👉 Fix `.ss-lock` CSS, then debug splash per Session 14 row (cache-busted incognito repro FIRST, console SECOND). Boss is testing: classic page → click "Raconteur+" in the cream toggle → expect Mesher-gradient splash, letter-by-letter "Raconteur+", 1s hold on "+", curtain lift.
